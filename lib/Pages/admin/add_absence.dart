...
class AddAbsence extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => AddAbsenceState();
}

enum DurationType { prolonge, journee }

class AddAbsenceState extends State<AddAbsence> {
  final String today = DateFormat('dd/MM/yyyy').format(DateTime.now());
  String? month;
  int? year;
  DurationType _duration = DurationType.journee;
        ...
        child: Form(
          child: Column(
            ...
            children: <Widget>[
              ...
              SizedBox(height: size.height * 0.01),
              _duration == DurationType.journee
                  ? Row(
  ...
  Future<void> _saveDataDays() async {
    final userData = await getCurrentUserData();
    String idA = generateId();
    String idN = generateId();
    try {
      FirebaseFirestore.instance.collection('Absence').doc(idA).set({
        'id': idA,
        'employeeId': _selectedEmployee,
        'type': 'days',
        'date1': formattedDate1,
        'date2': formattedDate2,
        'days': calculateDays(_selectedDate1!, _selectedDate2!),
        'year': year,
        'month': month,
      });
      FirebaseFirestore.instance.collection('UserNotification').doc(idN).set({
        ...
        'date': today,
        'content':
            'Une absence a été soumise de "$formattedDate1" à "$formattedDate2"',
        'isRead': false,
      });
  ...
  Future<void> _saveDataDay() async {
    final userData = await getCurrentUserData();
    String idA = generateId();
    String idN = generateId();
    try {
      ...
      FirebaseFirestore.instance.collection('UserNotification').doc(idN).set({
        ...
        'timestamp': DateTime.now(),
        'date': today,
        'content': 'Une absence a été soumise le "$formattedDate"',
        'isRead': false,
      });
      FirebaseFirestore.instance
          .collection('User')
          .doc(_selectedEmployee)
          .update({
        'Sanctions': userData.sanctions + 1,
        'resteConge': userData.soldeConge +
            userData.soldeAnneePrec -
            userData.congePris -
            1
  ...
  Future<void> _handleSubmit() async {
    if (_selectedEmployee != null) {
      switch (_duration) {
        case (DurationType.journee):
          if (formattedDate.isEmpty) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Veuillez séléctionner la date d\'absence.'),
              ),
            );
          } else {
            _saveDataDay();
          }
        case (DurationType.prolonge):
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Veuillez séléctionner l\'employé.'),
        ),
      );
    }
  }
